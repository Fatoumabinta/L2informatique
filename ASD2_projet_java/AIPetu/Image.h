...
public:
  ...
  /// 1 <= i <= height(), 1 <= j <= width()
  /// col is the new colortoIndex(int i,int j)
  ...
  //void readAIP(const std::string& filename);
  ...
  bool areConsecutivePixels(int i1, int j1, int i2, int j2) const;        //ok

  Color** getMatrix();

private:
    int width_;// width of this image
    int height_; // height of this image
    int size_; // size of this image
    Color** color_tab; // two-dimensional table to represent the matrix
